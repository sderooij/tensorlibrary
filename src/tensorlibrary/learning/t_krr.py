...
from ._cp_krr import get_system_cp_krr, CPKM_predict, CPKM_predict_batchwise, get_system_cp_LMPROJ
from ._cp_km import init_CP, _solve_TSVM_square_hinge, _init_model_params, _init_model_params_LMPROJ
...
class CPKRR(ClassifierMixin, BaseTKRR):
    ...
    def fit(self, x: tl.tensor, y: tl.tensor, **kwargs):
        ...
        if self._extra_reg:
            rank_init = self.w_init[0].shape[1]
            extra_reg = tl.ones((self.max_rank, rank_init))
        ...
        if self.train_loss_flag:
            if not balanced:
                ...
                self.train_loss.append(self._balanced_loss_fun(x, y, w, Cp, Cn, idx_p, idx_n))
        ...
        for it in range(itemax):
            ...
            if not balanced:
                ...
                AA, bb = get_system_cp_krr(z_x, G, y)
            else:
                z_x_n = self._features(x[idx_n, d])
                z_x_p = self._features(x[idx_p, d])
                reg /= w[d].T @ w[d]
                Gn /= z_x_n @ w[d]
                Gp /= z_x_p @ w[d]
                AA_n, bb_n = get_system_cp_krr(z_x_n, Gn, y[idx_n])
                AA_p, bb_p = get_system_cp_krr(z_x_p, Gp, y[idx_p])
                AA = Cn * AA_n + Cp * AA_p
                bb = Cn * bb_n + Cp * bb_p
            ...
            if self.loss == "l2":
                if self.debug:
                    ...
                    if not tl.all(np.linalg.eigvals(AA + reg_mat) > 0):
                        raise ValueError("Loss matrix is not positive definite")
                w_d = tl.solve(AA + reg_mat, bb)
    ...
    def _balanced_loss_fun(self, x, y, w, Cp, Cy, idx_p, idx_n):
        x_pos = x[idx_p,:]
        x_neg = x[idx_n,:]
        y_pos = y[idx_p]
        y_neg = y[idx_n]
        pos_loss = tl.norm(y_pos - CPKM_predict(x_pos, w, self._features)) ** 2
...
class CPKRR_LMPROJ(CPKRR):
    ...
    def fit(self, x: tl.tensor, y: tl.tensor, x_target=None):
        ...
        """
        min_w[d] ||y-<G, w[d]>||_F^2 + (reg_par)<W[d]^T W[d], H> + mu ||<gamma*P, w[d]>||_F^2
        ...
        # Initialize parameters
        if self.class_weight is None or self.class_weight == "none":
            H, G, P = _init_model_params_LMPROJ(x, w, self._features, x_target)
            balanced = False
        elif self.class_weight == "balanced":
            balanced = True
        ...
        # set gamma
        gamma = tl.concatenate([(1/N_s) * tl.ones((N_s, 1)), -(1/N_t) * tl.ones((N_t, 1))])
        ...
        for it in range(itemax):
            ...
            if balanced:
                ...
                z_x = self._features(x[:, d])
                G /= z_x @ w[d]  # remove current factor
                P /= tl.concatenate([z_x, z_x_target]) @ w[d]
            ...
            if balanced:
                ...
                GG, Gy = get_system_cp_krr(z_x, G, y, batch_size=self.batch_size)
                PP = get_system_cp_LMPROJ(
                    tl.concatenate([z_x, z_x_target]), P, gamma, batch_size=self.batch_size
                )

            # Solve system and assign to w[d]
            A = GG + self.reg_par * N * tl.kron(H.T, tl.eye(self.M)) + self.mu * N* PP
            b = Gy
            w_d = tl.solve(A, b)
            w[d] = tl.reshape(w_d, (self.M, self.max_rank), order="F")
